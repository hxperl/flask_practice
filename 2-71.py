...
class SQLAlchemySessionInterface(SessionInterface):
    ...
    def save_session(self, app, session, response):
        ...
        expires = self.get_expirtion_time(app, session)

        response.set_cookie(app.session_cookie_name, sessio.sid, expires= expires, httponly = httponly, domain = domain, secure = secure)

app = Flask(__name__)
app.session_interface = SQLAlchemySessionInterface()
app.config.update( SECRET_KEY='aaaaaa', SESSION_COOKIE_NAME='jpub_flask_session')
